...
export const addLogo = catchAsync(async (req, res) => {
  ...
  let lenders = await lendingInstitutionService.getLendingInstitutionList({ createdBy: { $exists: false } });
  ...
  const promises = await lenders.map((lender) => {
    ...
    return lendingInstitutionService.updateLendingInstitutionDetails(
      { _id: lender._id },
      {
        logo: { fileName, url: encodeUrl(logo) },
        // logo: {},
      }
    );
  });
  ...
  return res.status(httpStatus.CREATED).send({ results: 'Logos added to lender' });
});
