module.exports = {
  ...
  async up(db) {
    ...
    const usersAddedAsBorrower = await User.find({ sponsor: { $exists: true } }).toArray();
    ...
    const SponsorHavingNotBorrowers = await Sponsor.find({
      _id: { $nin: sponsorIdsHavingBorrowers },
    }).toArray();
    ...
    const dealsHavingSponsorWithNoBorrower = await Deal.updateMany(
      {
        sponsor: { $in: SponsorIdsHavingNotBorrowers },
      },
      { $unset: { sponsor: 1 } }
    );
    ...
    const deletedSponsor = await Sponsor.deleteMany({
      _id: { $in: SponsorIdsHavingNotBorrowers },
    });

    console.log(
      `Number of deals updated : ${
        dealsHavingSponsorWithNoBorrower.acknowledged ? dealsHavingSponsorWithNoBorrower.modifiedCount : 0
      } & Number of sponsors removed : ${deletedSponsor.acknowledged ? deletedSponsor.deletedCount : 0}`
    );
  },
  async down() {
