module.exports = {
  async up(db) {
    ...
    await Promise.all([
      deal.map(async (item) => {
        const lenderPlacement = await LenderPlacement.find({ deal: item._id, terms: { $exists: true } }).toArray();
        lenderPlacement.map(async (placement, index) => {
          await LenderPlacement.updateOne({ _id: placement._id }, { $set: { orderOfTerms: index } });
        });
      }),
    ]);
  },
