...
export const createLenderPlacement = {
  body: Joi.object().keys({
    lendingDetails: Joi.array().items(Joi.object().keys({
...
export const updateLenderPlacement = {
  body: Joi.object().keys({
    ...
    stage: Joi.string().valid(...Object.values(enumFields.EnumStageOfLenderPlacement)),
    terms: termsEmbed,
    termSheet: Joi.string().regex(new RegExp(`https://${config.aws.bucket}.s3.amazonaws.com\\b([-a-zA-Z0-9()@:%_+.~#?&amp;/=]*.(pdf|doc|docx|ppt|xls|xlsx|pptx)$)`))
  }),
...
export const sendDeal = {
  body: Joi.object().keys({
    deals: Joi.array().items(Joi.object().keys({
