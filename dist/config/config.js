...
const envVarsSchema = Joi.object().keys({
  ...
  APPLICATION_ENV: Joi.string().valid(...Object.values(EnumOfNodeEnv)).required(),
  ...
  ENABLE_PRIVATE_ACCESS: Joi.boolean().description('Flag to enable/disable PRIVATE access of uploaded files').default(false),
  ...
  DEFAULT_ADVISORS_TO_ADD_DEAL: Joi.string().required().default(['max@parallelcre.com', 'ag@parallelcre.com', 'richard@parallelcre.com']),
  // 1 hr = 3600000
  ...
  JWT_VERIFY_EMAIL_EXPIRATION_MINUTES: Joi.number().required().description('minutes after which email verification token expires'),
  // 24 hr = 86400000
  ...
  DATA_ENCRYPTION: Joi.boolean().description('Flag to enable/disable encryption and decryption of response and request').required(),
  RESET_PASSWORD_EXPIRATION_MINUTES: Joi.number().default(60).description('minutes after which password reset tokens expires'),
