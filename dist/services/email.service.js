...
export const sendEmail = async emailParams => {
  ...
  if (attachments) {
    msg.attachments = await Promise.all(msg.attachments.map(async item => {
...
<br>
  ...
  <div>Please <a class="text-center" target="_blank" href="${config.front.url}/${link}" >click here</a> to access the deal</div><br><br>
  <div>Thank You,</div>
  <div>The Parallel Team</div>
  </div>
  </body>
...
`;
  ...
  await sendEmailUsingGmail({
    from: fromEmail,
    pass: decrypt(pass, config.encryptionPassword),
...
export const sendOtpVerificationEmail = async (user, otp) => {
  ...
  await sendEmail({
    ...
    text,
    isHtml: false
  }).then(() => logger.info('email sent successfully')).catch(error => logger.warn(`Unable to send mail ${error}`));
};