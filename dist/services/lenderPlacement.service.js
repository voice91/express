...
export async function createLenderPlacement(body) {
  if (body.lendingInstitution) {
    ...
    if (lenderPlacement) {
      throw new ApiError(httpStatus.BAD_REQUEST, 'Some lending institutes are included in the deal, making it impossible to add them back, whereas others are added.');
    }
  }
  const lendingInstitution = await LendingInstitution.findOne({
...
export const removeLenderPlacementAssociatedThings = async lenderPlacement => {
  ...
  await Promise.all([taskService.removeManyTask(filterForTask), lenderNotesService.removeManyLenderNotes({
