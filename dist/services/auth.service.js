...
import { userService, tokenService, emailService } from "./";
...
export const verifyEmail = async verifyRequest => {
  ...
  await Token.deleteMany({
    user,
    type: EnumTypeOfToken.VERIFY_EMAIL
  });
  const filter = {
    _id: user
  };
  const findBorrower = await User.findById({
    _id: user
  });
  const userIsBorrower = await Invitation.find({
