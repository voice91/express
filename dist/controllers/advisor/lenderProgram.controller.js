  ...
  Object.keys(fields).forEach(field => {
    if (field === 'loanSize') {
      ...
      filter.lenderInstitute = {
        $in: lenderInstitute.map(item => item._id)
      };
    } else {
      filter[field] = {
        $in: [fields[field]]
...
export const addLender = catchAsync(async (req, res) => {
  ...
  const getlenderInstitute = await LendingInstitution.findOne({
...
export const editLender = catchAsync(async (req, res) => {
  ...
  await LendingInstitution.findOneAndUpdate({
    _id: lenderInstitute
  }, {
    lenderType: body.lender.lenderType
  });
  const lenderProgram = await Promise.all(body.lenderProgram.map(async item => {
    if (item.minLoanSize && item.maxLoanSize) {
      if (item.minLoanSize >= item.maxLoanSize) {
        throw new ApiError(httpStatus.BAD_REQUEST, 'Minimum Loan Amount is Greater than Maximum Loan Amount..!!');
      }
    ...
    return LenderProgram.findOneAndUpdate({
      _id: item.id || mongoose.Types.ObjectId()
    }, item, {
      upsert: true,
      new: true
    });
