...
export const list = catchAsync(async (req, res) => {
  ...
  const fields = pick(query, ['loanType', 'propertyType', 'statesArray', 'lenderInstitute', 'loanSize', 'lenderType', 'lenderNameVisible']);
  let lenderProgram = [];
  ...
  Object.keys(fields).forEach(field => {
    if (filterFields.includes(field)) {
      filter._id = {
        $in: lenderProgram.map(item => item.lenderInstitute)
      };
  ...
  const lenderPlacements = await lenderPlacementService.getLenderPlacementList({
    deal: dealId
  });
  const lendingInstitutionsWithProgramData = lendingInstitution.map(lendingInstitutionObject => {
    ...
    return {
      ...
      isAlreadyAdded: lenderPlacements.some(lender => lender.lendingInstitution.toString() === lendingInstitutionObject._id.toString()),
