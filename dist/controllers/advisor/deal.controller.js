...
import { activityLogService, dealDocumentService, dealNotesService, dealService, dealSummaryService, invitationService, lenderNotesService, lenderPlacementService, taskService, userService } from "../../services";
...
export const update = catchAsync(async (req, res) => {
  ...
  if (dealSummaryBody) {
    ...
    await dealSummaryService.updateDealSummary({
...
export const remove = catchAsync(async (req, res) => {
  ...
  await Promise.allSettled([dealSummaryService.removeDealSummary(filterToRemove), dealDocumentService.removeDealDocument(filterToRemove), dealNotesService.removeManyDealNotes(filterToRemove), invitationService.removeManyInvitation(filterToRemove), taskService.removeManyTask(filterToRemove),
  // eslint-disable-next-line array-callback-return
  ...lenderPlacement.map(item => {
    lenderNotesService.removeManyLenderNotes({
      lenderPlacement: item._id
    });
  }), lenderPlacementService.removeManyLenderPlacement(filterToRemove)]).then(results => results.forEach(result => {
...
export const dealInvitation = catchAsync(async (req, res) => {
  ...
  const existingUsers = await userService.getUserList({
    email: {
      $in: body.email
    }
  });
