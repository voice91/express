...
import { s3Service, lenderPlacementService, emailService, emailTemplateService, activityLogService, lenderContactService, dealService, userService, invitationService, tokenService } from "../../services";
import { catchAsync } from "../../utils/catchAsync";
import FileFieldValidationEnum from "../../models/fileFieldValidation.model";
import mongoose from 'mongoose';
import TempS3 from "../../models/tempS3.model";
import { asyncForEach, checkTermAdded, encodeUrl, getEmailSubjectForDeal, getStateFullName, getTextFromTemplate, manageDealStageTimeline, manageLenderPlacementStageTimeline, sendDealTemplate, constructEmailContent } from "../../utils/common";
...
import enumModel, { EnumOfActivityType, EnumOfEmailStatus, EnumOfNodeEnv, EnumStageOfDeal, EnumStageOfLenderPlacement } from "../../models/enum.model";
import config from "../../config/config";
import { stageOfLenderPlacementWithNumber, lenderPlacementStageToStageNumberMapping } from "../../utils/enumStageOfLenderPlacement";
...
export const getEmailDataV3 = catchAsync(async (req, res) => {
  ...
  const hasSameDealId = lenderPlacements.every(placement => placement.deal.id === lenderPlacements[0].deal.id && placement.lenderContact);
  if (!hasSameDealId) {
...
export const sendEmailV3 = catchAsync(async (req, res) => {
  ...
  if (sendToAdvisor) {
    ...
    await emailService.sendEmailUsingGmail({
      ...
      subject: `TEST - ${getEmailSubjectForDeal(dealDetail.dealSummary)}`,
  ...
  await Promise.all(req.body.lenderPlacementIds.map(async lenderPlacementId => {
    const options = {
      populate: [{
        path: 'lenderContact'
      }, {
    ...
    if (config.application_env !== EnumOfNodeEnv.SANDBOX) {
      const response = await emailService.sendEmailUsingGmail({
        ...
        cc: ccList,
        subject: isFollowUp ? `RE: ${getEmailSubjectForDeal(dealDetail.dealSummary)}` : `${getEmailSubjectForDeal(dealDetail.dealSummary)}`,
        // calling common function for setting subject
        ...
        text: isFollowUp ? `${getFollowUpContent()}${constructEmailContent({
          lenderPlacement,
  ...
  const lenderPlacementsAssociatedWithDeal = await lenderPlacementService.getLenderPlacementList({
    deal: dealId
  });

  // Filter placements with a stage lower than the target placement stage
  const placementWithTargetStage = lenderPlacementsAssociatedWithDeal.filter(placement => lenderPlacementStageToStageNumberMapping.get(placement.stage) < lenderPlacementStageToStageNumberMapping.get(EnumStageOfLenderPlacement.SENT));
  ...
  const deal = await Deal.findByIdAndUpdate(dealId, dealDataToUpdate, {
    new: true
  });
...
export const list = catchAsync(async (req, res) => {
  ...
  const filter = {
    ...
    stage: query.stage ? query.stage : {
      $ne: EnumStageOfLenderPlacement.ARCHIVE
    }
  };
  const options = {
    ...
    populate: [{
      ...
      match: {
        notesType: enumModel.EnumOfNotesTypeOfLenderNotes.INTERNAL_NOTE
      }
    }],
...
export const create = catchAsync(async (req, res) => {
  ...
  await Promise.all(body.lendingDetails.map(async lendingInstitute => {
    const placement = {
      ...lendingInstitute,
...
export const update = catchAsync(async (req, res) => {
  ...
  if ([enumModel.EnumStageOfLenderPlacement.CLOSING, enumModel.EnumStageOfLenderPlacement.CLOSED].includes(lenderPlacementResult.stage)) {
    const {
      stage
...
export const sendEmail = catchAsync(async (req, res) => {
  ...
  if (sendToAdvisor) {
    const isAdvisor = _.template(getEmailTemplate.emailContent)({
      ...
      purchasePrice: (parseFloat(String(_.find(dealDetail.loanInformation, data => data?.key === 'purchasePrice')?.value)?.replaceAll(/[$,]/g, '')) || 0) / 1000000 || '[x.xx purchasePrice]',
      inPlaceNOI: (parseFloat(String(_.find(dealDetail.loanInformation, data => data?.key === 'inPlaceNOI')?.value)?.replaceAll(/[$,]/g, '')) || 0) / 1000000 || '[x.xx]',
      stabilizedNOI: (parseFloat(String(_.find(dealDetail.loanInformation, data => data?.key === 'stabilizedNOI')?.value)?.replaceAll(/[$,]/g, '')) || 0) / 1000000 || '[x.xx]',
      passLink: '',
      dealSummaryLink: ''
    });
  ...
  const getText = (passLink, dealSummaryLink) => {
    ...
    const data = _.template(getEmailTemplate.emailContent)({
      ...
      // eslint-disable-next-line no-shadow
      String(_.find(dealDetail?.loanInformation, data => data?.key === 'purchasePrice')?.value)?.replaceAll(/[$,]/g, '')) || 0) / 1000000 || '[x.xx]',
      inPlaceNOI: (parseFloat(
      // eslint-disable-next-line no-shadow
      String(_.find(dealDetail?.loanInformation, data => data?.key === 'inPlaceNOI')?.value)?.replaceAll(/[$,]/g, '')) || 0) / 1000000 || '[x.xx]',
      stabilizedNOI: (parseFloat(
      // eslint-disable-next-line no-shadow
      String(_.find(dealDetail?.loanInformation, data => data?.key === 'stabilizedNOI')?.value)?.replaceAll(/[$,]/g, '')) || 0) / 1000000 || '[x.xx]',
      dealSummaryLink: `<a href=${dealSummaryLink}>Deal Summary</a>`,
      passLink: `<a href=${passLink}>Pass</a>`
    });
    return data;
  ...
  await Promise.allSettled(getEmailTemplate.contact.map(async (item, index) => {
    const frontEndUrl = config.front.url || 'http://54.196.81.18';
  ...
  const result = await LenderPlacement.findOne({
    _id: placementId
  });
...
export const sendDealV2 = catchAsync(async (req, res) => {
  ...
  const promises = await deals.map(async body => {
    ...
    const filter = isFollowUp ? {
      lenderPlacement,
      templateName: {
        $in: [`advisorFollowUpTemplate - ${emailBodyValues.lenderName}`, `borrowerFollowUpTemplate - ${emailBodyValues.lenderName}`]
      }
    } : {
      lenderPlacement,
      templateName: {
        $in: [`advisorSendDealTemplate - ${emailBodyValues.lenderName}`, `borrowerSendDealTemplate - ${emailBodyValues.lenderName}`]
      }
    };
    emailTemplate = await emailTemplateService.getOne(filter, {
      populate: 'deal'
    });
    ...
    if (!emailTemplate) {
      emailTemplate = await EmailTemplate.create({
        ...
        isEmailSent: false,
        totalLoanAmount,
        templateName: isFollowUp ? `advisorFollowUpTemplate - ${emailBodyValues.lenderName}` : `advisorSendDealTemplate - ${emailBodyValues.lenderName}`
      });
    }
    ...
    const result = await lenderPlacementService.getOne({
      _id: placementId
    });
    ...
    const dealData = await dealService.updateDeal({
      ...
      stage,
      orderOfStage: stageOfDealWithNumber(stage),
      timeLine: manageDealStageTimeline(lenderContact.lenderPlacement.deal.stage, stage, lenderContact.lenderPlacement.deal.timeLine),
      nextStep: enumModel.EnumNextStepOfLenderPlacement[stage],
...
export const sendMessage = catchAsync(async (req, res) => {
  ...
  if (config.application_env !== EnumOfNodeEnv.SANDBOX) {
    response = await emailService.sendEmailUsingGmail({
      ...
      text: `${body.message}${constructEmailContent({
        lenderPlacement,
        sender: req.user.sendEmailFrom
      })}`,
...
export const getMessages = catchAsync(async (req, res) => {
  ...
  const lenderPlacement = await lenderPlacementService.updateLenderPlacement(filter, {
    $addToSet: {
