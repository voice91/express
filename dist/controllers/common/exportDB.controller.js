...
import { CsvReverseLenderLoanTypeMapping, CsvReverseLenderPropertyTypeMapping, CsvReverseLenderTypeMapping, CsvStatesArrayMapping } from "../../utils/common";
...
export const exportToExcel = catchAsync(async (req, res) => {
  ...
  const fieldNamesofLenderContact = ['Lender', 'First Name', 'Last Name', 'Program(s)', 'Nick Name', 'Email', 'Main Phone', 'Mobile Phone', 'Office Phone', 'Title', 'City', 'State', 'contactTag', 'Email Tag', 'ContactId', 'LenderId'];
  ...
  const lenderNotes = await LenderInstituteNotes.find({}, {}, {
    populate: 'createdBy'
  }).lean();
  ...
  lenderProgram.forEach(item => {
    ...
    const notes = lenderNotes.filter(note => note.lenderInstitute.toString() === lender._id.toString()).sort((a, b) => b.createdAt - a.createdAt);
    let {
      statesArray
    } = item;
  ...
  for (let i = 2; i <= lastRow; i += 1) {
    LenderProgramsheet.getCell(`${getColumnReferenceFromColumnName(fieldNamesofLenderProgram, 'Min')}${i}`).numFmt = '_("$"* #,##0_);_("$"* (#,##0);_("$"* "-"??_);_(@_)'; // format cell of min amount as number with $
    LenderProgramsheet.getCell(`${getColumnReferenceFromColumnName(fieldNamesofLenderProgram, 'Max')}${i}`).numFmt = '_("$"* #,##0_);_("$"* (#,##0);_("$"* "-"??_);_(@_)'; //  format cell of max amount as number with $
    LenderProgramsheet.getCell(`${getColumnReferenceFromColumnName(fieldNamesofLenderProgram, 'Non-Recourse LTV')}${i}`).numFmt = '0%'; //  format cell of nonRecourseLTV as percentage
  }
