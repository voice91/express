...
export const processEmailMessage = catchAsync(async (req, res) => {
  ...
  const message = req.body.StrippedTextReply ? `<body>${messageContent[1].replace(/<\/div>$/, '')}</body>` : 'This file is coming from email';
  let msgId;
  if (req.body.Headers) {
  ...
  const placement = await LenderPlacement.findOne({
    postmarkMessageId: {
      $elemMatch: {
        $eq: msgId[0]
      }
