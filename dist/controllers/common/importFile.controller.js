...
import { CsvLenderLoanTypeMapping, CsvLenderPropertyTypeMapping, CsvLenderTypeMapping, CsvStatesArrayMapping, isObjectId, processDateForExcel } from "../../utils/common";
...
const processLenderProgramAndInstitutionData = async (lenderWorkbook, user) => {
  ...
  const programValue = Object.entries(lenderWorkbookSheet).find(([, value]) => value.v === LenderWorkBookExcelEnum.LENDER_NAME);
  ...
  while (true) {
    ...
    const lenderName = lenderWorksheet.getCell(currentRowNo, currentCell.col + LenderWorkBookKeyColMappingForInstitute.LENDER_NAME);
    const lenderType = lenderWorksheet.getCell(currentRowNo, currentCell.col + LenderWorkBookKeyColMappingForInstitute.LENDER_TYPE);
    const programName = lenderWorksheet.getCell(currentRowNo, currentCell.col + LenderWorkBookKeyColMappingForInstitute.PROGRAM_NAME);
    program.lenderProgramType = programName.value;
    const programMinLoanSize = lenderWorksheet.getCell(currentRowNo, currentCell.col + LenderWorkBookKeyColMappingForInstitute.PROGRAM_MIN_LOAN_SIZE);
    if (programMinLoanSize.value) {
      if (programMinLoanSize.value < 100000 || programMinLoanSize.value > 1000000000) {
        throw new Error(`minLoanSize must be a containing from 100000 to 1000000000 row:${currentRowNo} col: ${currentCell.col + LenderWorkBookKeyColMappingForInstitute.PROGRAM_MIN_LOAN_SIZE}`);
      }
    ...
    const programMinTag = lenderWorksheet.getCell(currentRowNo, currentCell.col + LenderWorkBookKeyColMappingForInstitute.PROGRAM_MIN_TAG);
    if (programMinTag.value) {
      if (programMinTag.value < 1 || programMinTag.value > 5) {
        throw new Error(`minLoanSizeTag must be a containing numbers from 1 to 5 row:${currentRowNo} col: ${currentCell.col + LenderWorkBookKeyColMappingForInstitute.PROGRAM_MIN_TAG}`);
      }
    }
    program.minLoanTag = programMinTag.value;
    const programMaxLoanSize = lenderWorksheet.getCell(currentRowNo, currentCell.col + LenderWorkBookKeyColMappingForInstitute.PROGRAM_MAX_LOAN_SIZE);
    if (programMaxLoanSize.value) {
      if (programMaxLoanSize.value < 100000 || programMaxLoanSize.value > 1000000000) {
        throw new Error(`maxLoanSize must be a containing from 100000 to 1000000000 row:${currentRowNo} col: ${currentCell.col + LenderWorkBookKeyColMappingForInstitute.PROGRAM_MAX_LOAN_SIZE}`);
      }
    ...
    const programMaxTag = lenderWorksheet.getCell(currentRowNo, currentCell.col + LenderWorkBookKeyColMappingForInstitute.PROGRAM_MAX_TAG);
    if (programMaxTag.value) {
      if (programMaxTag.value < 1 || programMaxTag.value > 5) {
        throw new Error(`maxLoanSizeTag must be a containing numbers from 1 to 5 row:${currentRowNo} col: ${currentCell.col + LenderWorkBookKeyColMappingForInstitute.PROGRAM_MAX_TAG}`);
      }
    }
    program.maxLoanTag = programMaxTag.value;
    const programStates = lenderWorksheet.getCell(currentRowNo, currentCell.col + LenderWorkBookKeyColMappingForInstitute.PROGRAM_STATES);
    ...
    const programPropertyType = lenderWorksheet.getCell(currentRowNo, currentCell.col + LenderWorkBookKeyColMappingForInstitute.PROGRAM_PROPERTY_TYPE);
    ...
    const loanType = lenderWorksheet.getCell(currentRowNo, currentCell.col + LenderWorkBookKeyColMappingForInstitute.LOAN_TYPE);
    if (loanType.value) {
    ...
    const index = lenderWorksheet.getCell(currentRowNo, currentCell.col + LenderWorkBookKeyColMappingForInstitute.INDEX_USED);
    program.indexUsed = index.value;
    const spread = lenderWorksheet.getCell(currentRowNo, currentCell.col + LenderWorkBookKeyColMappingForInstitute.SPREAD_ESTIMATION);
    program.spreadEstimate = spread.value;
    const counties = lenderWorksheet.getCell(currentRowNo, currentCell.col + LenderWorkBookKeyColMappingForInstitute.COUNTIES);
    if (counties.value) {
      program.counties = counties.value.split(',').map(item => item.trim());
    }
    const recourse = lenderWorksheet.getCell(currentRowNo, currentCell.col + LenderWorkBookKeyColMappingForInstitute.RECOURSE);
    program.recourseRequired = recourse.value;
    const nonRecourse = lenderWorksheet.getCell(currentRowNo, currentCell.col + LenderWorkBookKeyColMappingForInstitute.NON_RECOURSE);
    program.nonRecourseLTV = nonRecourse.value;
    const lenderData = {};
    const description = lenderWorksheet.getCell(currentRowNo, currentCell.col + LenderWorkBookKeyColMappingForInstitute.DESCRIPTION);
    const headquarters = lenderWorksheet.getCell(currentRowNo, currentCell.col + LenderWorkBookKeyColMappingForInstitute.HEADQUARTERS);
    ...
    const lenderId = lenderWorksheet.getCell(currentRowNo, currentCell.col + LenderWorkBookKeyColMappingForInstitute.INSTITUTE_ID);
    const obj = {
    ...
    if (lenderId.value) {
      // eslint-disable-next-line no-await-in-loop
      const updatedLender = await LendingInstitution.findByIdAndUpdate(lenderId.value, obj, {
        new: true
      });
      if (!updatedLender) {
        Object.assign(obj, {
      ...
      // eslint-disable-next-line no-await-in-loop
      const findInstitute = await LendingInstitution.findOneAndUpdate({
        lenderNameVisible: lenderName.value
      }, obj);
    ...
    for (let i = 1; i <= 5; i += 1) {
      ...
      const noteContent = lenderWorksheet.getCell(currentRowNo, currentCell.col + LenderWorkBookKeyColMappingForInstitute[`NOTE_${i}_CONTENT`]);
      const noteId = lenderWorksheet.getCell(currentRowNo, currentCell.col + LenderWorkBookKeyColMappingForInstitute[`NOTE_${i}_ID`]);
      const noteDate = lenderWorksheet.getCell(currentRowNo, currentCell.col + LenderWorkBookKeyColMappingForInstitute[`NOTE_${i}_DATE`]);
      if (noteId.value) {
    ...
    const programId = lenderWorksheet.getCell(currentRowNo, currentCell.col + LenderWorkBookKeyColMappingForInstitute.PROGRAM_ID);
...
const processLenderContactData = async lenderWorkbook => {
  ...
  while (true) {
    const contact = {};
    const lender = lenderContactWorksheet.getCell(currentRowNo, currentCell.col + LenderWorkBookKeyColMappingForContact.LENDER_NAME);
    if (lender.value) {
      const firstName = lenderContactWorksheet.getCell(currentRowNo, currentCell.col + LenderWorkBookKeyColMappingForContact.FIRST_NAME);
      contact.firstName = firstName.value;
      const lastName = lenderContactWorksheet.getCell(currentRowNo, currentCell.col + LenderWorkBookKeyColMappingForContact.LAST_NAME);
      contact.lastName = lastName.value;
      const program = lenderContactWorksheet.getCell(currentRowNo, currentCell.col + LenderWorkBookKeyColMappingForContact.PROGRAMS);
      ...
      const nickName = lenderContactWorksheet.getCell(currentRowNo, currentCell.col + LenderWorkBookKeyColMappingForContact.NICK_NAME);
      contact.nickName = nickName.value;
      const email = lenderContactWorksheet.getCell(currentRowNo, currentCell.col + LenderWorkBookKeyColMappingForContact.EMAIL);
      if (email.value) {
        if (typeof email.value === 'string') {
          ...
          throw new Error(httpStatus.BAD_REQUEST, `Please provide valid Email row:${currentRowNo} col: ${currentCell.col + LenderWorkBookKeyColMappingForContact.EMAIL}`);
        }
      } else {
        notAvailableLender.push(contact);
      }
      const mainPhone = lenderContactWorksheet.getCell(currentRowNo, currentCell.col + LenderWorkBookKeyColMappingForContact.MAIN_PHONE_NUMBER);
      contact.phoneNumberDirect = mainPhone.value;
      const mobilePhone = lenderContactWorksheet.getCell(currentRowNo, currentCell.col + LenderWorkBookKeyColMappingForContact.MOBILE_PHONE_NUMBER);
      contact.phoneNumberCell = mobilePhone.value;
      const officePhone = lenderContactWorksheet.getCell(currentRowNo, currentCell.col + LenderWorkBookKeyColMappingForContact.OFFICE_PHONE_NUMBER);
      contact.phoneNumberOffice = officePhone.value;
      const title = lenderContactWorksheet.getCell(currentRowNo, currentCell.col + LenderWorkBookKeyColMappingForContact.TITLE);
      contact.title = title.text;
      const city = lenderContactWorksheet.getCell(currentRowNo, currentCell.col + LenderWorkBookKeyColMappingForContact.CITY);
      contact.city = city.value;
      const state = lenderContactWorksheet.getCell(currentRowNo, currentCell.col + LenderWorkBookKeyColMappingForContact.STATE);
      if (state.value) {
        contact.state = state.value;
      }
      const contactTag = lenderContactWorksheet.getCell(currentRowNo, currentCell.col + LenderWorkBookKeyColMappingForContact.CONTACT_TAG);
      if (contactTag.value) {
      ...
      const emailTag = lenderContactWorksheet.getCell(currentRowNo, currentCell.col + LenderWorkBookKeyColMappingForContact.EMAIL_TAG);
      if (emailTag.value) {
      ...
      const contactId = lenderContactWorksheet.getCell(currentRowNo, currentCell.col + LenderWorkBookKeyColMappingForContact.CONTACT_ID);
      if (contactId.value) {
        contact.contactId = mongoose.Types.ObjectId(contactId.value);
      }
      const lenderId = lenderContactWorksheet.getCell(currentRowNo, currentCell.col + LenderWorkBookKeyColMappingForContact.LENDER_INSTITUTE);
      if (lenderId.value) {
        contact.lenderInstitute = lenderId.value;
        // eslint-disable-next-line no-await-in-loop
        const findInstitute = await LendingInstitution.findOne({
        ...
        if (!findInstitute) {
          // eslint-disable-next-line no-await-in-loop
          await LendingInstitution.findByIdAndUpdate(lenderId.value, {
            lenderNameVisible: lender.value
          });
        ...
        // eslint-disable-next-line no-await-in-loop
        const findInstitution = await LendingInstitution.findOne({
          lenderNameVisible: lender.value
        });
        if (findInstitution) {
    ...
    if (contact.contactId) {
      ...
      contact.emailTag = contact.emailTag ? contact.emailTag : 1;
      // eslint-disable-next-line no-await-in-loop
      const registeredLender = await userService.getOne({
        email: contact.email
      });
      if (!registeredLender) {
        const userBody = {
      ...
      // eslint-disable-next-line no-await-in-loop
      const updatedContact = await LenderContact.findOneAndUpdate({
        email: contact.email
      }, contact, {
        new: true
      });
...
export const importDataFromFile = catchAsync(async (file, res) => {
  try {
    ...
    const lenderWorkbook = XLSX.read(data, {
      type: 'buffer'
    });
