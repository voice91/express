...
export const list = catchAsync(async (req, res) => {
  ...
  const lenderPlacement = lenderPlacements.filter(placement => placement.lenderAllContacts.map(contact => contact.email).includes(req.user.email));
  return res.status(httpStatus.OK).send({
    results: lenderPlacement
  });
});
export const paginate = catchAsync(async (req, res) => {
  ...
  const deal = await dealService.getDealList({
