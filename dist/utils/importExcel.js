...
import { EnumColumnNameOfFinancialSummary, EnumOfTypeOfValue, EnumSheetNameOfUw, EnumTableNameInUwSheet } from "../models/enum.model";
...
function formatMathFormulaFormValue({
  val,
  key,
  tableName
}) {
 ...
 */
function retrieveAndFormatTableValues({
  excelSheetData,
  currentCell,
  tableName
}) {
  ...
  while (true) {
    ...
    let result = formatMathFormulaFormValue({
      val: value.value,
      key: key.value,
      tableName
    });
 ...
 * @return {Array} resultArray - Array of retrieved and formatted data.
 */
function processSimpleTableData({
  summaryData,
  tableName,
  excelSheetData
}) {
 ...
 */
function processFinancialSummaryData({
  excelSheetData,
  currentCell,
  headerOne,
  headerTwo,
  ...
  while (true) {
    ...
    if (value.value !== EnumColumnNameOfFinancialSummary.IN_PLACE && value.value !== EnumColumnNameOfFinancialSummary.STABILIZED) {
      data.key = key.value;
  ...
  try {
    ...
    workbook.eachSheet(excelSheetData => {
      if (excelSheetData.name === EnumSheetNameOfUw.SUMMARY) {
        ...
        // Sources and Uses
        const sourcesUsesValue = Object.entries(summaryData).find(([, value]) => value.v === EnumTableNameInUwSheet.SOURCES_AND_USES);
        const sources = [];
        const uses = [];
        if (sourcesUsesValue) {
        ...
        const financialSummaryValue = Object.entries(noIData).find(([, value]) => value.v === EnumTableNameInUwSheet.FINANCIAL_SUMMARY);
        // Financial Summary
        if (financialSummaryValue) {
          ...
          logger.info(`${totalRevenue.length} rows imported for ${EnumTableNameInUwSheet.REVENUE} in ${EnumTableNameInUwSheet.FINANCIAL_SUMMARY}`);
          ...
          if (expensesValue) {
            ...
            logger.info(`${expenses.length} rows imported for ${EnumTableNameInUwSheet.EXPENSES} in ${EnumTableNameInUwSheet.FINANCIAL_SUMMARY}`);
            const effectiveGrossIncomeInPlace = totalRevenue.find(revenue => revenue.key === EnumColumnNameOfFinancialSummary.EFFECTIVE_GROSS_INCOME).inPlaceValue;
            const effectiveGrossIncomeStabilized = totalRevenue.find(revenue => revenue.key === EnumColumnNameOfFinancialSummary.EFFECTIVE_GROSS_INCOME).stabilizedValue;
            const totalOperatingExpensesInPlace = expenses.find(expense => expense.key === EnumColumnNameOfFinancialSummary.TOTAL_OPERATING_EXPNESES).inPlaceValue;
            const totalOperatingExpensesStabilized = expenses.find(expense => expense.key === EnumColumnNameOfFinancialSummary.TOTAL_OPERATING_EXPNESES).stabilizedValue;
        ...
        const rentRollSummaryValue = Object.entries(rentRollSheetData).find(([, value]) => value.v === EnumTableNameInUwSheet.RENT_ROLL_SUMMARY);
        // Rent Roll Summary
  ...
  try {
    ...
    workbook.eachSheet(excelSheetData => {
      ...
      if (matchingPair && matchingPair.length === 1) {
        ...
        const matchingKeyValuePair = {
          [titleIndex]: customBlockTableData[titleIndex]
        };
