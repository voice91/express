...
export const detailsInDeal = async (stageName, dealId) => {
  ...
  if (stageName === EnumStageOfDeal.OUT_IN_MARKET) {
    lenderPlacementNumber = await LenderPlacement.find({
      deal: dealId,
      stageEnumWiseNumber: {
        $lte: stageOfLenderPlacementWithNumber(EnumStageOfLenderPlacement.SENT)
      }
    }).count();
  }
  let lenderPlacementQuotes;
  if (stageName === EnumStageOfDeal.SELECTING_LENDER) {
    lenderPlacementQuotes = await LenderPlacement.find({
      deal: dealId,
      terms: {
        $exists: true
      }
  ...
  if (stageName === EnumStageOfDeal.CLOSING) {
    lenderPlacementClosing = await LenderPlacement.find({
      deal: dealId,
      stage: 'closing'
    }).populate({
  ...
  const stageMapping = new Map([[EnumStageOfDeal.NEW, 'Waiting on initial information needed from borrower'], [EnumStageOfDeal.PREPARING_MATERIALS, 'Advisor is preparing materials'], [EnumStageOfDeal.OUT_IN_MARKET, `Outreach to ${lenderPlacementNumber} Lenders`], [EnumStageOfDeal.SELECTING_LENDER, `${lenderPlacementQuotes} Quotes Received`], [EnumStageOfDeal.CLOSING, `In closing with ${lenderPlacementClosing.map(name => name.lendingInstitution.lenderNameVisible)}`]]);
